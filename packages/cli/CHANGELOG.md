...
- 3d96752: Fix `wald grow` to serve `src/assets/*` files (a project's own global stylesheet, images, etc.) at their `config.base`-prefixed URL — previously, a non-default `base` made a freshly-scaffolded project's global stylesheet link 404 in dev, loading the site completely unstyled.
...
- 8420dd7: Fix `wald grow` to respect `config.base` when routing incoming requests — previously, a non-default `base` (e.g. `/my-forest/`) made every page 404 in dev, since the dev server's routing never accounted for the prefix Vite itself was already correctly adding everywhere else.
