...
export async function scaffold(targetDir: string): Promise<void> {
  ...
  writeFileSync(
    ...
    [
      ...
      "// SafeHtml opts out of the runtime's default HTML-escaping — safe here since this HTML is built from your own local markdown files, not untrusted input",
  ...
  writeFileSync(
    ...
    [
      ...
      "// SafeHtml opts out of the runtime's default HTML-escaping — safe here since post.body is rendered markdown from your own local content, not untrusted input",
      '---',
      '<Layout title={post.data.title}>',
      '  <article class="post-body">',
      '    <h1>{post.data.title}</h1>',
      '    {new SafeHtml(post.body)}',
      '  </article>',
      '</Layout>',
      '',
    ].join('\n')
